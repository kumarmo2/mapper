...
fn main() {
    ...
    let outer_dest = OuterDest::from(outer_source);
    let mut list = outer_dest.source.list.iter();
    assert_eq!(Some(&"1".to_string()), list.next());
    assert_eq!(Some(&"2".to_string()), list.next());
    assert_eq!(Some(&"3".to_string()), list.next());
